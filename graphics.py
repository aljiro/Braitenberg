...
class MainWindow(QMainWindow):
	...
	def __init__(self, experiment, *args, **kwargs):
		...
		self.label_title = QLabel( "Braitenberg vehicles simulation" )
		self.label_title.setAlignment( Qt.AlignCenter )
		self.label_title.setFixedHeight( 20 )

		# Creating the painting context
		self.scanvas = SimulationCanvas( self, 880, 500 )		
	...
	def draw( self, model ):
		...
		pm = "(paused)" if self.experiment.simulation.is_pause else ""
		self.statusb.showMessage( "Simulation running t = " + str(self.experiment.simulation.t) + pm )
	...
	def onStopClick( self, s ):
		self.experiment.simulation.stop()

	def onPauseClick( self, s ):
		self.experiment.simulation.pause()
	...
	def runSimulation( self, maxT ):
		...
		self.experiment.start( xmin, xmax, maxT)
		# self.initPlots( self.simulation.model )

		self.experiment.simulation.signals.draw.connect( self.draw )
		self.experiment.simulation.signals.start.connect( self.start )
		self.experiment.simulation.signals.finish.connect( self.finish )
		self.experiment.simulation.signals.pause.connect( self.pause )
		# self.experiment.simulation.signals.plottingCompleted.connect( self.simulation.resume )

		self.threadpool.start( self.experiment.simulation )
